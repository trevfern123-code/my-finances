...
🟢 **Phase 1 + Phase 2a live in production, verified, and all 3 items encrypted under a rotated key (`RAILWAY_PROD_V2`)** (2026-08-31) — schema migration applied, dual-write/dual-read encryption running against real traffic, the two originally plaintext-only items backfilled, and a full V1 → V2 key rotation completed after the V1 key was accidentally exposed and treated as compromised. See §21 of `PLAID_TOKEN_ENCRYPTION_DESIGN_REVIEW.md` for the original Phase 2a production verification and §23 for the rotation/backfill completion record. **Currently in a short post-rotation soak**: `RAILWAY_PROD_V1` remains configured in Railway as a deliberate, temporary safety margin (no production row still needs it — see §25 for the exact post-soak removal checklist). Phase 2b (encrypted-only writes for new items) has **not** started — a revised proposal exists (§24 of the design doc) but is not approved or implemented; every existing and newly-linked item still gets both representations. Full design in `PLAID_TOKEN_ENCRYPTION_DESIGN_REVIEW.md` (read that before touching any of this — this section is a pointer, not the source of truth).
...
**Done**: the Phase 1 migration is applied to production Supabase; Phase 2a is deployed and live-verified (§21); the two originally plaintext-only items were backfilled and, along with the third item, rotated from `RAILWAY_PROD_V1` to `RAILWAY_PROD_V2` after V1 was accidentally exposed (§23) — all 3 rows now encrypted under V2, plaintext still present on all 3 (Phase 2a dual-write unchanged by the rotation), zero anomalies. Two purpose-built, Codex-audited one-off scripts did this work: `backend/src/scripts/rotateTartanTokenKey.ts` and a retargeted `backend/src/scripts/backfillTokenEncryption.ts` — both inert (never imported by the running server, guarded by `require.main === module`).

**Not yet done, and deliberately not started without your explicit go-ahead**: removing `RAILWAY_PROD_V1` from Railway once the current soak concludes (§25 has the exact checklist), and Phase 2b (flipping to encrypted-only writes for newly-linked items) — a revised proposal exists (§24 of the design doc) but is not approved or implemented.
