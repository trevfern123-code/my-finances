...
🟢 **Phase 1 + Phase 2a live in production, verified** (2026-08-28) — schema migration applied, keys configured in Railway (Sealed Variables), and dual-write/dual-read encryption is running against real traffic: existing plaintext-only items keep working unchanged, and a newly linked item was confirmed dual-written (plaintext + all 5 encrypted columns) with successful refresh, sync, and webhook processing. See §21 of `PLAID_TOKEN_ENCRYPTION_DESIGN_REVIEW.md` for the exact production verification record. Phase 2b (encrypted-only) and the backfill of the two pre-existing plaintext-only items have **not** started — proposed, not yet approved; nothing plaintext-related has been touched. Full design in `PLAID_TOKEN_ENCRYPTION_DESIGN_REVIEW.md` (read that before touching any of this — this section is a pointer, not the source of truth).
...
**Done**: the Phase 1 migration is applied to production Supabase; the production key (`RAILWAY_PROD_V1`) is configured in Railway as a Sealed Variable; Phase 2a is deployed and live-verified — see §21 of the design doc for the full production verification record (integrity query, dual-write confirmation, refresh/sync/webhook checks for both plaintext-only and newly-encrypted items).

**Not yet done, and deliberately not started without your explicit go-ahead**: Phase 2b (flipping to encrypted-only writes) and the backfill of the two remaining plaintext-only items. A Phase 2b/backfill proposal has been written (§22 of the design doc) but not approved or implemented.
