...
🟡 **In progress** — Phase 1 (schema) and Phase 2a (dual-write, encryption-aware code) are implemented and fully unit-tested locally; nothing has been applied to production yet. Full design in `PLAID_TOKEN_ENCRYPTION_DESIGN_REVIEW.md` (read that before touching any of this — this section is a pointer, not the source of truth).
...
**Not yet done, and deliberately not started without your explicit go-ahead**: applying the Phase 1 migration to production, generating/storing the real staging and production keys, Railway configuration, Phase 2a's live verification, Phase 2b (flipping to encrypted-only writes), and the backfill/verification/cleanup phases after that. See the design doc's §19 and the handoff message for the exact manual steps.
