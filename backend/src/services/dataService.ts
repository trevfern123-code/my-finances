...
// One attempt = one server-created Hosted Link token, stored ENCRYPTED (tokenEncryption.ts's
// encryptLinkToken, AAD-bound to the attempt id), bound to the user AND login session that asked
// for it, completable once. The link token is never returned to a client and never logged. See
// supabase/migrations/20260922130000_plaid_link_attempts.sql for the lifecycle
// (pending -> claimed -> exchanging -> completed | failed | exchange_unknown).

/** SHA-256 (hex) of a link token — how a SESSION_FINISHED webhook finds its attempt without the
 *  stored ciphertext ever being decrypted for it. */
export function hashLinkToken(linkToken: string): string {
  return createHash('sha256').update(linkToken, 'utf8').digest('hex');
}

/** Refused by create_plaid_link_attempt: five of this user's attempts are already being completed. */
export class TooManyPlaidLinkAttemptsError extends Error {
  constructor() {
    super('Too many bank links are still being finished. Wait a moment and try again.');
    this.name = 'TooManyPlaidLinkAttemptsError';
  }
}

/** Stores a new pending attempt `attemptId` (generated by the caller, since the encryption AAD
 *  needs it first) and returns its database-clock expiry. */
export async function createPlaidLinkAttempt(params: {
  attemptId: string;
  userId: string;
  sessionId: string;
  linkToken: string;
}): Promise<{ expiresAt: string }> {
  const enc = encryptLinkToken(params.linkToken, getKeyRing(), params.attemptId);
  const { data, error } = await supabaseAdmin.rpc('create_plaid_link_attempt', {
    p_id: params.attemptId,
    p_user_id: params.userId,
    p_session_id: params.sessionId,
    p_link_token_hash: hashLinkToken(params.linkToken),
    p_link_token_ciphertext: enc.ciphertextBase64,
    p_link_token_nonce: enc.nonceBase64,
    p_link_token_auth_tag: enc.authTagBase64,
    p_link_token_key_id: enc.keyId,
    p_link_token_enc_version: enc.encVersion,
  });
  if (error) {
    if (error.code === 'P0001' && error.message.includes('too many Plaid Link attempts in progress')) {
      throw new TooManyPlaidLinkAttemptsError();
    }
    throw new Error(`Failed to start Plaid Link attempt: ${error.message}`);
  }
  if (typeof data !== 'string' || data === '') throw new Error('Failed to start Plaid Link attempt: no expiry returned');
  return { expiresAt: data };
}

const PLAID_LINK_ATTEMPT_STATUSES = ['pending', 'claimed', 'exchanging', 'completed', 'failed', 'exchange_unknown'] as const;
export type PlaidLinkAttemptStatus = (typeof PLAID_LINK_ATTEMPT_STATUSES)[number];

/** A pending or claimed attempt carries its decrypted link token (for /link/token/get); later ones
 *  do not — it has been erased. `stale`: a claim or exchange older than two minutes (its process is
 *  presumed gone). */
export type PlaidLinkAttemptState =
  | { status: 'pending' | 'claimed'; expired: boolean; stale: boolean; linkToken: string }
  | { status: Exclude<PlaidLinkAttemptStatus, 'pending' | 'claimed'>; expired: boolean; stale: boolean };

interface PlaidLinkAttemptRow {
  status: string;
  expired: boolean;
  stale: boolean;
  link_token_ciphertext: string | null;
  link_token_nonce: string | null;
  link_token_auth_tag: string | null;
  link_token_key_id: string | null;
  link_token_enc_version: number | null;
}

/**
 * The attempt as its OWN user in its OWN login session sees it; null for anyone else, or for an
 * unknown id. Decrypting the link token can throw a PlaidCredentialError (fixed, non-sensitive
 * message) — never a fallback.
 */
export async function readPlaidLinkAttempt(
  attemptId: string,
  userId: string,
  sessionId: string
): Promise<PlaidLinkAttemptState | null> {
  const { data, error } = await supabaseAdmin.rpc('read_plaid_link_attempt', {
    p_id: attemptId,
    p_user_id: userId,
    p_session_id: sessionId,
  });
  if (error) throw new Error(`Failed to read Plaid Link attempt: ${error.message}`);
  if (!Array.isArray(data)) throw new Error('Failed to read Plaid Link attempt: unexpected result');
  if (data.length === 0) return null;
  if (data.length !== 1) throw new Error('Failed to read Plaid Link attempt: unexpected result');

  const row = data[0] as PlaidLinkAttemptRow;
  if (
    !PLAID_LINK_ATTEMPT_STATUSES.includes(row.status as PlaidLinkAttemptStatus) ||
    typeof row.expired !== 'boolean' ||
    typeof row.stale !== 'boolean'
  ) {
    throw new Error('Failed to read Plaid Link attempt: unexpected result');
  }
  const status = row.status as PlaidLinkAttemptStatus;
  if (status !== 'pending' && status !== 'claimed') return { status, expired: row.expired, stale: row.stale };

  if (
    row.link_token_ciphertext === null ||
    row.link_token_nonce === null ||
    row.link_token_auth_tag === null ||
    row.link_token_key_id === null ||
    row.link_token_enc_version === null
  ) {
    throw new PartialEncryptedRepresentationError(attemptId);
  }
  const linkToken = decryptLinkToken(
    {
      ciphertextBase64: row.link_token_ciphertext,
      nonceBase64: row.link_token_nonce,
      authTagBase64: row.link_token_auth_tag,
      keyId: row.link_token_key_id,
      encVersion: row.link_token_enc_version,
    },
    getKeyRing(),
    attemptId
  );
  return { status, expired: row.expired, stale: row.stale, linkToken };
}

export type PlaidLinkClaim =
  | { outcome: 'claimed'; claimToken: string }
  | { outcome: 'invalid' | 'expired' | 'in_progress' | 'completed' | 'failed' | 'exchange_unknown' };
const PLAID_LINK_CLAIM_REFUSALS: readonly string[] = ['invalid', 'expired', 'in_progress', 'completed', 'failed', 'exchange_unknown'];

/**
 * Claims the caller's own attempt (pending, or a stale claim whose exchange never began) with a
 * fresh claim token; a stale EXCHANGE is recorded as exchange_unknown instead, never re-claimed.
 * Only 'claimed' lets the caller go on; anything the function does not return verbatim is an error.
 */
export async function claimPlaidLinkAttempt(attemptId: string, userId: string, sessionId: string): Promise<PlaidLinkClaim> {
  const { data, error } = await supabaseAdmin.rpc('claim_plaid_link_attempt', {
    p_id: attemptId,
    p_user_id: userId,
    p_session_id: sessionId,
  });
  if (error) throw new Error(`Failed to claim Plaid Link attempt: ${error.message}`);
  const row = Array.isArray(data) && data.length === 1 ? (data[0] as { outcome?: unknown; claim_token?: unknown }) : null;
  if (row?.outcome === 'claimed' && typeof row.claim_token === 'string' && row.claim_token !== '') {
    return { outcome: 'claimed', claimToken: row.claim_token };
  }
  if (typeof row?.outcome === 'string' && PLAID_LINK_CLAIM_REFUSALS.includes(row.outcome) && row.claim_token === null) {
    return { outcome: row.outcome as Exclude<PlaidLinkClaim['outcome'], 'claimed'> };
  }
  throw new Error('Failed to claim Plaid Link attempt: unexpected result');
}

/** Durably records, for the claim holder only, that the exchange may be about to start. Must return
 *  true BEFORE /item/public_token/exchange is called; false means the claim was lost. */
export async function beginPlaidLinkExchange(
  attemptId: string,
  userId: string,
  sessionId: string,
  claimToken: string
): Promise<boolean> {
  const { data, error } = await supabaseAdmin.rpc('begin_plaid_link_exchange', {
    p_id: attemptId,
    p_user_id: userId,
    p_session_id: sessionId,
    p_claim_token: claimToken,
  });
  if (error) throw new Error(`Failed to begin Plaid Link exchange: ${error.message}`);
  if (typeof data !== 'boolean') throw new Error('Failed to begin Plaid Link exchange: unexpected result');
  return data;
}

/** The atomic store's outcome could not be determined (no response, or a 5xx): the item may or may
 *  not have been stored. The access token must then be neither removed nor retried. */
export class PlaidLinkStoreOutcomeUnknownError extends Error {
  constructor() {
    super('The outcome of storing the linked item is unknown');
    this.name = 'PlaidLinkStoreOutcomeUnknownError';
  }
}

/**
 * Stores the newly exchanged Item (access token ENCRYPTED, AAD-bound to a fresh plaid_items id; no
 * plaintext) and completes the attempt in ONE database transaction (store_plaid_link_item):
 *  - 'stored'   the item is durable and the attempt completed
 *  - 'refused'  the attempt is no longer this claim's exchange (nothing stored)
 *  - 'rejected' the database answered with an error (4xx): the transaction rolled back, nothing stored
 * Throws PlaidLinkStoreOutcomeUnknownError when there was no definitive answer.
 */
export async function storePlaidLinkItem(params: {
  attemptId: string;
  userId: string;
  sessionId: string;
  claimToken: string;
  plaidItemId: string;
  accessToken: string;
}): Promise<{ outcome: 'stored'; itemRowId: string } | { outcome: 'refused' | 'rejected' }> {
  const itemRowId = randomUUID();
  const enc = encryptAccessToken(params.accessToken, getKeyRing(), itemRowId);
  const { data, error, status } = await supabaseAdmin.rpc('store_plaid_link_item', {
    p_id: params.attemptId,
    p_user_id: params.userId,
    p_session_id: params.sessionId,
    ...
    p_access_token_ciphertext: enc.ciphertextBase64,
    p_access_token_nonce: enc.nonceBase64,
    p_access_token_auth_tag: enc.authTagBase64,
    p_access_token_key_id: enc.keyId,
    p_access_token_enc_version: enc.encVersion,
  });
  if (error) {
    if (typeof status === 'number' && status >= 400 && status < 500) return { outcome: 'rejected' };
    throw new PlaidLinkStoreOutcomeUnknownError();
  }
  if (data === true) return { outcome: 'stored', itemRowId };
  if (data === false) return { outcome: 'refused' };
  throw new PlaidLinkStoreOutcomeUnknownError();
}

export type PlaidLinkFailureReason =
  | 'exited'
  | 'ambiguous'
  | 'exchange_rejected'
  | 'exchange_outcome_unknown'
  | 'store_failed_item_removed'
  | 'store_failed_remove_unknown';

/** Ends an attempt that linked nothing here, with its reason (fail_plaid_link_attempt allows each
 *  reason only from the states it describes). False when the transition does not apply. */
export async function failPlaidLinkAttempt(
  attemptId: string,
  userId: string,
  sessionId: string,
  claimToken: string | null,
  reason: PlaidLinkFailureReason
): Promise<boolean> {
  const { data, error } = await supabaseAdmin.rpc('fail_plaid_link_attempt', {
    p_id: attemptId,
    p_user_id: userId,
    p_session_id: sessionId,
    p_claim_token: claimToken,
  ...
  if (error) throw new Error(`Failed to record Plaid Link failure: ${error.message}`);
  if (typeof data !== 'boolean') throw new Error('Failed to record Plaid Link failure: unexpected result');
  return data;
}

/** For a verified SESSION_FINISHED webhook: records readiness once. Informational only. */
export async function markPlaidLinkAttemptReady(linkToken: string, status: string): Promise<boolean> {
  const { data, error } = await supabaseAdmin.rpc('mark_plaid_link_attempt_ready', {
    p_link_token_hash: hashLinkToken(linkToken),
    p_status: status,
  });
  if (error) throw new Error(`Failed to record Plaid Link readiness: ${error.message}`);
  return data === true;
}

/** Institution enrichment for an already-stored item — follow-up work after linking, also retried
 *  by refreshAccounts for any item still missing it. */
export async function updatePlaidItemInstitution(
  ...
  const { error } = await supabaseAdmin
    ...
    .eq('id', itemRowId);
  if (error) throw new Error(`Failed to update item institution: ${error.message}`);
}

/** Ids of this user's items that have no institution recorded yet. */
export async function getPlaidItemIdsMissingInstitution(userId: string): Promise<Set<string>> {
  const { data, error } = await supabaseAdmin.from('plaid_items').select('id').eq('user_id', userId).is('institution_id', null);
  if (error) throw new Error(`Failed to load Plaid items: ${error.message}`);
