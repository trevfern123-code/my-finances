...
const PLAID_LINK_ATTEMPT_STATUSES = ['pending', 'completing', 'completed', 'failed'] as const;
export type PlaidLinkAttemptStatus = (typeof PLAID_LINK_ATTEMPT_STATUSES)[number];

/** A pending attempt carries its decrypted link token (for /link/token/get); a later one does not
 *  — its token has been erased. */
export type PlaidLinkAttemptState =
  | { status: 'pending'; expired: boolean; linkToken: string }
  | { status: Exclude<PlaidLinkAttemptStatus, 'pending'>; expired: boolean };
 ...
 * unknown id. Decrypting a pending attempt's link token can throw a PlaidCredentialError (fixed,
 * non-sensitive message) — never a fallback.
 */
export async function readPlaidLinkAttempt(
  ...
  if (!PLAID_LINK_ATTEMPT_STATUSES.includes(row.status as PlaidLinkAttemptStatus) || typeof row.expired !== 'boolean') {
    throw new Error('Failed to read Plaid Link attempt: unexpected result');
  }
  const status = row.status as PlaidLinkAttemptStatus;
  if (status !== 'pending') return { status, expired: row.expired };
  ...
  return { status, expired: row.expired, linkToken };
}

export type PlaidLinkClaimOutcome = 'claimed' | 'invalid' | 'expired' | 'completing' | 'completed' | 'failed';
const PLAID_LINK_CLAIM_OUTCOMES: readonly string[] = ['claimed', 'invalid', 'expired', 'completing', 'completed', 'failed'];

/** Atomically moves the caller's own pending, unexpired attempt to 'completing'. Only 'claimed' lets
 *  the caller exchange; anything the function does not return verbatim is an error. */
export async function claimPlaidLinkAttempt(
  attemptId: string,
  userId: string,
  sessionId: string
): Promise<PlaidLinkClaimOutcome> {
  ...
  if (typeof data !== 'string' || !PLAID_LINK_CLAIM_OUTCOMES.includes(data)) {
    throw new Error('Failed to claim Plaid Link attempt: unexpected result');
  }
  return data as PlaidLinkClaimOutcome;
}

/** Ends the attempt and erases its stored link token: 'completed' (from completing, with the stored
 *  item's id) or 'failed' (from pending or completing). False when the transition does not apply. */
export async function finishPlaidLinkAttempt(
  attemptId: string,
  userId: string,
  sessionId: string,
  ...
  const { data, error } = await supabaseAdmin.rpc('finish_plaid_link_attempt', {
    p_id: attemptId,
    p_user_id: userId,
    p_session_id: sessionId,
    p_outcome: outcome,
    p_plaid_item_id: plaidItemRowId,
  });
  if (error) throw new Error(`Failed to finish Plaid Link attempt: ${error.message}`);
  if (typeof data !== 'boolean') throw new Error('Failed to finish Plaid Link attempt: unexpected result');
  return data;
}
